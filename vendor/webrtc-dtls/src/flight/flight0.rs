...
use crate::config::*;
use crate::conn::*;
use crate::curve::named_curve::select_supported_curve;
            ...
            for extension in &client_hello.extensions {
                match extension {
                    Extension::SupportedEllipticCurves(e) => {
                        // Take the first offered curve we actually implement
                        // rather than blindly taking `elliptic_curves[0]`.
                        //
                        // Modern Chrome/Edge advertise post-quantum groups
                        // (e.g. X25519MLKEM768) ahead of the classical ones.
                        // Those decode to `NamedCurve::Unsupported`, so the
                        // unconditional assignment let an unsupported curve
                        // through and keypair generation later failed with
                        // `invalid named curve` -- a fatal alert, no SRTP
                        // keys, and therefore no media at all.
                        //
                        // `None` covers both an empty list and a list with no
                        // curve in common; both fail with the same alert.
                        state.named_curve = match select_supported_curve(&e.elliptic_curves) {
                            Some(curve) => curve,
                            None => {
                                return Err((
                                    Some(Alert {

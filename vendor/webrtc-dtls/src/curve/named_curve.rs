...
/// Pick the first curve in `offered` that this implementation supports.
///
/// Callers must not assume `offered[0]` is usable: peers are free to advertise
/// groups we do not implement (current Chrome/Edge list the post-quantum hybrid
/// X25519MLKEM768 first), and every unrecognised value decodes to
/// [`NamedCurve::Unsupported`]. Selecting one of those makes keypair
/// generation fail with `invalid named curve`, which aborts the DTLS
/// handshake and leaves the connection with no SRTP keys.
///
/// Returns `None` when there is no curve in common, including the empty case.
